...
Quill.register(
  `formats/${ClassificationBlot.blotName}`,
  ClassificationBlot,
  true,
);
...
export const NewRichTextEditor2 = memo(
  forwardRef(
    (
      props: NewRichTextEditorProps,
      forwardedRef: Ref<RichTextEditorHandler>,
    ) => {
      const {
        id,
      ...
      const prevClassificationConfig = useRef<
        RichTextClassificationConfig | undefined
      >(classificationConfig);

      useEffect(() => {
        if (classificationConfig?.enabled) {
          const classificationModule = quillRef.current?.editor?.getModule(
            "classificationModule",
          ) as any;
          if (
            classificationModule &&
            !isEqual(prevClassificationConfig.current, classificationConfig)
          ) {
      ...
      const handlers = useMemo(() => {
        return {
          ...
          align(value: any) {
            ...
            const resizeModule = quillRef.current?.editor?.getModule(
              "resize",
            ) as QuillResize;
        ...
        return {
          ...
          source: async (
            ...
            if (fetchMentionUsers) {
              ...
              renderList(
                matchedPeople.map((people) => ({
                  ...people,
                  value: `@${people.username}`,
                })),
              );

              const mentionListContainerElement = document.querySelectorAll(
                ".ql-mention-list-container",
              );
              if (
                mentionListContainerElement &&
                mentionListContainerElement.length > 0
              ) {
          ...
          onSelect: (item: any, insertItem: Function) => {
            const totalCharsLength = (
              quillRef.current?.editor?.getModule(
                "maxCharsLimit",
              ) as MaxCharsLimitModule
            ).totalCharsLength;
      ...
      const findNextFocusableElement = (
        ...
        const focusableElementsIdentifier =
          "a:not(.ql-tooltip a, [disabled], [data-embeddable]), " +
        ...
        const focusableElements = Array.from(
          parent.querySelectorAll(focusableElementsIdentifier),
        ).filter((element: any) => {
          return (
        ...
        const hasNextFocusableElement =
          focusableElements.length > 0 && index + 1 < focusableElements.length;
      ...
      const focusNextElement = (editor: any) => {
        ...
        if (quillEditor) {
          const nextFocusableElement = modalElement
            ? findNextFocusableElement(quillEditor, modalElement)
            : findNextFocusableElement(quillEditor, formElement);
      ...
      const addEmbeddedImage = (url: string, altText?: string) => {
        if (quillRef.current && quillRef.current.getEditor()) {
          quillRef.current.getEditor().insertEmbed(
            insertImageCurrentIndexRef.current,
            "image",
            {
              src: url,
              alt: altText,
          ...
          quillRef.current
            .getEditor()
            .insertText(insertImageCurrentIndexRef.current + 1, "\n", "user");
          setTimeout(() => {
            if (quillRef.current?.getEditor()) {
              quillRef.current
                .getEditor()
                ?.setSelection(insertImageCurrentIndexRef.current + 2, 0);
      ...
      const imagePasteHandler = async (
        _dataUrl: string | ArrayBuffer,
        _type: string,
        imageData: ImageData,
      ) => {
        if (uploadImage) {
      ...
      const modules = useMemo(
        () => ({
          ...
          counter: characterLeftLabel && {
            maxCharacter: maxCharacterLimit,
            label: characterLeftLabel,
          },
          // mention: enableAtMention ? getMentionConfiguration() : false,
          markdownShortcuts: {},
          classificationModule: { configData: classificationConfig },
          maxCharsLimit: (maxCharacterLimit || characterLeftLabel) && {
            maxCharacters: maxCharacterLimit,
          },
          resize: {
      ...
      const isEmptyHtml = (htmlString: any) => {
        const emptyRegex =
          /^(<(p|h1|h2|h3|h4|h5|h6)\s*(class=["'][\w-\s]+["'])?>(<br>|<br\/>|<br\s\/>|\s+)<\/(p|h1|h2|h3|h4|h5|h6)>)+$/gm;
      ...
      const onChangeEvent = useCallback(
        (
          ...
          if (classificationConfig?.enabled && value?.length) {
            const parsedDocs: Document = new DOMParser().parseFromString(
              value,
              "text/html",
            );
            const allClassificationLabels = parsedDocs.querySelectorAll(
      ...
      useImperativeHandle(forwardedRef, () => {
        const setText = (value: string) =>
          quillRef.current?.editor?.setText(value, "user");

        const setHtml = (value: string) => {
          const deltas = quillRef.current?.editor?.clipboard.convert({
            html: value,
          });
          if (deltas) {
        ...
        return {
          ...
          hasInvalidClassifications() {
            ...
            return (
              classificationModule?.hasInvalidClassificationInputs() ?? true
            );
          },
          isCharacterLimitExceeded(): boolean {
            const remainingCharactersModule =
              quillRef.current?.editor?.getModule(
                "counter",
              ) as RemainingCharactersModule;
          ...
          getHtmlContent() {
            return (
              (quillRef.current?.getEditor()?.scroll?.domNode as any)
                ?.innerHTML || ""
            );
          },
        };
      ...
      useEffect(() => {
        RichTextEditorHelper.fixQuillAccessibility(
          id,
          ariaLabelledBy,
          ariaLabel,
        );
      ...
      useEffect(() => {
            ...
            if (emojiPopup) {
              ...
              if (style && +style.top.replace("px", "") < 0) {
                if (window.innerWidth < 420) {
                  ...
                  (emojiPopup as HTMLElement).style.top =
                    -(+style.height.replace("px", "") + 30) + "px";
                }
              }
            }
          });
        ...
        const emojiPopupPosition = () => {
          const emojiControlElements = document.querySelectorAll(
          ...
          if (emojiControlElements) {
            Array.from(emojiControlElements).forEach((element: Element) => {
              element.addEventListener("click", () => {
                const emojiPopups =
                  document.querySelectorAll("#textarea-emoji");
                if (emojiPopups) {
        ...
        Array.from(editorElement).forEach((el) => {
          el.querySelectorAll(".textarea-emoji-control").forEach(
            (emojiControl) => {
          ...
          if (modalElm) {
            modalElm.addEventListener("click", (event) => {
              ...
              const emojiButton = clickedElement.parentElement?.closest(
                ".textarea-emoji-control",
              );
              const emojiPlate =
                clickedElement.parentElement?.closest("#textarea-emoji");
              if (!emojiButton && !emojiPlate) {
                closeEmojiPlate();
              }

              const qlPicker = clickedElement.parentElement?.closest(
                ".ql-header.ql-picker",
              );
              if (!qlPicker) {
                const headerPicker = document.querySelector(
                  ".modal .ql-header.ql-picker",
                );
                if (headerPicker) {
        ...
        document.addEventListener("click", (event: MouseEvent) => {
          ...
          if (
            clickedElement.classList.contains("bem") &&
            clickedElement.classList.contains("ap")
          ) {
            closeEmojiPlate();
          }
          if (
            clickedElement.classList.contains("i-nature") ||
            clickedElement.classList.contains("i-people") ||
            clickedElement.classList.contains("i-food") ||
            clickedElement.classList.contains("i-symbols") ||
            clickedElement.classList.contains("i-activity") ||
            clickedElement.classList.contains("i-travel") ||
            clickedElement.classList.contains("i-objects") ||
            clickedElement.classList.contains("i-flags")
          ) {
            curseEmoji();
          }
        });
        ...
        >
          <ReactQuill
            ...
            onChange={(value, delta, source, editor) =>
              onChangeEvent(value, delta, source, editor)
            }
            onBlur={onBlur}
            onFocus={onFocus}
